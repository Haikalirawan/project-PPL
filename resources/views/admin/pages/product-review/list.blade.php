...
<div class="toolbar" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-5 align-items-center my-1"><span class="text-muted fw-normal">Home - Master Data - Feedback - </span>&nbsp;List Product Review</h1>
        </div>
    </div>
</div>
@endsection
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                ...
                <tbody class="fw-bold text-gray-600">
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="fw-bolder ms-3" data-kt-ecommerce-product-filter="category_name">{{$item->title}}</span>
                        </td>
                        <td class="text-center">
                            <span class="fw-bolder">{{$item->name}}</span>
                        </td>
                        <td class="text-center min-w-100px">{{$item->feedback}}</td>
                        <td class="text-center min-w-100px">{{$item->proof}}</td>
                        <td class="text-center min-w-100px">{{$item->rating}}</td>
                        <td class="text-center" data-order="{{$item->status}}">
                            <div class="badge @if ($item->status == 'agree') badge-light-success @elseif ($item->status == 'pending') badge-light-warning @else badge-light-danger @endif ">{{$item->status}}</div>
                        </td>
                        <td class="text-center">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-review/'.$item->id.'/pending-review')}}" method="POST" class="inline">
                                        @method('put')
                                        @csrf
                                        <button type="submit" class="btn w-100 menu-link px-3 fs-7 text-center">Pending</button>
                                    </form>
                                </div>
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-review/'.$item->id.'/agree-review')}}" method="POST" class="inline">
                                        @method('put')
                                        @csrf
                                        <button type="submit" class="btn w-100 menu-link px-3 fs-7 text-center">Setuju</button>
                                    </form>
                                </div>
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-review/'.$item->id.'/cancel-review')}}" method="POST" class="inline">
                                        @method('put')
                                        @csrf
                                        <button type="submit" class="btn w-100 menu-link px-3 fs-7 text-center">Tolak</button>
                                    </form>
                                </div>
                            </div>
                        </td>
