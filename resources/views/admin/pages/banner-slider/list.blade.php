...
<div class="toolbar" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-5 align-items-center my-1"><span class="text-muted fw-normal">Home - Master Data - Banner Slider - </span>&nbsp;List Banner Slider</h1>
        </div>
        <div class="d-flex align-items-center gap-2 gap-lg-3">
            <a href="{{url('/back-banner-slider/add-banner-slider')}}" class="btn btn-sm btn-primary">Tambah Data</a>
        </div>
    </div>
</div>
@endsection
        ...
        <div class="card-body pt-0">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                ...
                <tbody class="fw-bold text-gray-600">
                    @foreach ($getBannerSlider as $item)
                    <tr>
                        <td class="text-center">
                            <span class="fw-bolder text-center p-2">{{$loop->iteration}}</span>
                        </td>
                        <td class="text-left">
                            <span class="fw-bolder" data-kt-ecommerce-product-filter="category_name">{{ \Illuminate\Support\Str::limit($item->caption, 150, $end='...') }}</span>
                        </td>
                        <td class="text-center min-w-100px">
                            <span class="fw-bolder">
                                <span class="d-block bgi-no-repeat bgi-size-cover bgi-position-center card-rounded position-relative min-h-60px" style="background-image:url('{{asset('image/upload/banner-slider')}}/{{$item->image}}')">
                            </span>
                        </td>
                        <td class="text-center">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-banner-slider/'.$item->id.'/delete-banner-slider')}}" method="POST" class="inline">
                                        @method('delete')
                                        @csrf
