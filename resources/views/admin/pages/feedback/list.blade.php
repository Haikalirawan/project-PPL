...
<div class="toolbar" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-5 align-items-center my-1"><span class="text-muted fw-normal">Home - Master Data - Feedback - </span>&nbsp;List Feedback</h1>
        </div>
    </div>
</div>
@endsection
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                <thead>
                    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                        ...
                        <th class="text-center">Terakhir Ditambahkan</th>
                ...
                <tbody class="fw-bold text-gray-600">
                    ...
                    <tr>
                        ...
                        <td class="text-center min-w-100px">{{$item->message}}</td>
                        <td class="text-center min-w-100px">{{$item->created_at}}</td>
                        <td class="text-center" data-order="{{$item->status}}">
                            <div class="badge @if ($item->status == 'read') badge-light-success @else badge-light-warning @endif ">{{$item->status}}</div>
                        </td>
                        <td class="text-center">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-feedback/'.$item->id.'/update-feedback')}}" method="POST" class="inline">
                                        @method('put')
                                        @csrf
                                        <button type="submit" class="btn w-100 menu-link px-3 fs-7 text-center" onclick="return confirm('Tandai Sudah Dibaca ?')">Tandai Sudah Dibaca</button>
                                    </form>
                                </div>
                            </div>
                            @else
                            <a href="#" class="btn btn-sm btn-light btn-active-light-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Sudah Dibaca</a>
                            @endif
                        </td>
                    </tr>
                    @endforeach
