...
<div class="toolbar" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-5 align-items-center my-1"><span class="text-muted fw-normal">Home - Master Data - Product Category - </span>&nbsp;List Product Category</h1>
        </div>
        <div class="d-flex align-items-center gap-2 gap-lg-3">
            <a href="{{url('/back-product-category/add-product-category')}}" class="btn btn-sm btn-primary">Tambah Data</a>
        </div>
    </div>
</div>
@endsection
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                <thead>
                    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                        ...
                        <th class="text-center">Ditambahkan Tanggal</th>
                ...
                <tbody class="fw-bold text-gray-600">
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="fw-bolder">{{$item->color_code}}</span>
                            <div class="h-15px m-auto mt-2 rounded" style="background-color: {{$item->color_code}};"></div>
                        </td>
                        ...
                        <td class="text-center" data-order="{{$item->status}}">
                            <div class="badge @if ($item->status == 'release') badge-light-success @else badge-light-warning @endif ">{{$item->status}}</div>
                        </td>
                        <td class="text-center">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-product-category/'.$item->id.'/edit-product-category')}}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="btn w-100 menu-link px-3 fs-7 text-center">Edit</button>
                                    </form>
                                </div>
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-product-category/'.$item->id.'/delete-product-category')}}" method="POST" class="inline">
                                        @method('delete')
                                        @csrf
