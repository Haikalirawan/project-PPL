...
<div class="toolbar" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-5 align-items-center my-1"><span class="text-muted fw-normal">Home - Master Data - Product Gallery - </span>&nbsp;List Product Gallery</h1>
        </div>
        <div class="d-flex align-items-center gap-2 gap-lg-3">
            <a href="{{url('/back-product-gallery/add-product-gallery')}}" class="btn btn-sm btn-primary">Tambah Data</a>
        </div>
    </div>
</div>
@endsection
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                <thead>
                    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                        ...
                        <th class="text-center p-2">Caption</th>
                        <th class="text-center p-2">Opsi</th>
                    </tr>
                </thead>
                <tbody class="fw-bold text-gray-600">
                    @foreach ($getProductGallery as $item)
                    <tr>
                        <td class="text-center">
                            <span class="text-center p-2">{{$loop->iteration}}</span>
                        </td>
                        <td class="text-center">
                            <span class="text-center p-2" data-kt-ecommerce-product-filter="category_name">{{$item->title}}</span>
                        </td>
                        <td class="text-center">
                            <span class="d-block bgi-no-repeat bgi-size-cover bgi-position-center card-rounded position-relative min-h-60px" style="background-image:url('{{asset('image/upload/product-gallery')}}/{{$item->image}}')"></span>
                        </td>
                        <td class="text-center p-2">{{$item->caption}}</td>
                        <td class="text-center p-2">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-product-gallery/'.$item->id.'/delete-product-gallery')}}" method="POST" class="inline">
                                        @method('delete')
                                        @csrf
